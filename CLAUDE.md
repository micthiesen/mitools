...
- **Persistence** (`persistence/`) — three layers over one better-sqlite3 database per `DB_NAME` (WAL mode; path prefixed `/data/` when `DOCKERIZED`):
  - `docstore` owns the connection (`getDb()`) and stores CBOR-encoded blobs in a single `blobs` table: `pk`, `data`, plus metadata columns `entity`, `version`, `expires_at`, `updated_at`. New columns are added idempotently via a `PRAGMA table_info` guard in `initialize()`. Every read filters out expired rows (`expires_at <= now`); `cleanupExpired()` reclaims them physically.
  - `Entity<Data, PKProps>` layers documents on the docstore, keyed off the `entity` column (not pk prefixes). Keys use a length-prefixed, type-tagged codec (`$name#s3:abc#n5`) so components can't collide or coerce. Constructor takes either `(name, pkProps)` or an options object (`{ name, pk, version, migrate, validate, defaultTtlMs }`). API: get/getAll/upsert (with `ttlMs`/`expiresAt`)/patch/`update` (transactional)/delete/deleteAll/has/count/touch/`keys()` (returns structured pk objects). Deliberately no querying beyond pk. **Migration is one-shot, not lazy:** call `Entity.migrateAll()` once at startup (after `Injector.configure`) to rewrite existing rows into the current key encoding + payload `version` + metadata columns; reading pre-migration data is undefined behavior.
